...
async def create_product(product: ProductCreate):
    ...
    try:
        ...
        store_result = (
            supabase.table("stores").select("id").eq("name", product.store).execute()
        )

        if not store_result.data:
            raise HTTPException(
                status_code=400, detail=f"Store '{product.store}' not found"
            )
...
async def update_product(product_id: int, product: ProductUpdate):
    ...
    try:
        ...
        existing = (
            supabase.table("products").select("id").eq("id", product_id).execute()
        )
...
async def add_ml_product(
    request: MLProductAdd, current_user: Dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        logger.info(
            f"[ML Add] Produto encontrado: {product_info.get('title', 'N/A')[:50]}"
        )
        ...
        product_data = {
            ...
            "description": (
                product_info.get("subtitle", "")[:500]
                if product_info.get("subtitle")
                else None
            ),
            "affiliate_link": affiliate_link,
            "current_price": float(product_info["price"]),
            "original_price": (
                float(product_info.get("original_price"))
            ...
            "image_url": (
                product_info.get("pictures", [{}])[0].get("url")
                if product_info.get("pictures")
                else None
            ),
            "is_active": True,
        ...
        # 6. Buscar store_id
        store_result = (
            supabase.table("stores").select("id").eq("name", "mercado_livre").execute()
        )
        ...
        logger.info(
            f"[ML Add] Produto salvo com sucesso! ID: {result.data[0].get('id')}"
        )
        ...
        if e.response.status_code == 404:
            raise HTTPException(
                status_code=404, detail="Produto não encontrado no Mercado Livre"
            )
