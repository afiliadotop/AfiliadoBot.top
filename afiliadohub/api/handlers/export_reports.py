...
class ReportExporter:
    ...
    async def export_to_pdf(self, data: Dict, report_type: str) -> bytes:
        ...
        try:
            ...
            elements.append(
                Paragraph(f"Relatório AfiliadoHub - {report_type}", title_style)
            )
            elements.append(
                Paragraph(
            ...
            if "products" in data and data["products"]:
                ...
                for product in data["products"][:50]:
                    products_data.append(
                        [
                            ...
                            (
                                f"{product.get('discount_percentage', 0)}%"
                ...
                products_table = Table(
                    products_data, colWidths=[0.5 * inch, 3 * inch, inch, inch, inch]
                )
                ...
                if len(data["products"]) > 50:
                    elements.append(
                        Paragraph(
                            f"... e mais {len(data['products']) - 50} produtos",
                            self.styles["Italic"],
                        )
                    )
            ...
            if "by_store" in data and data["by_store"]:
                elements.append(
                    Paragraph("🏪 Análise por Loja", self.styles["Heading2"])
                )
                ...
                store_table = Table(
                    store_data, colWidths=[1.5 * inch, inch, 1.5 * inch, 1.5 * inch]
                )
            ...
            elements.append(
                Paragraph(
                    "AfiliadoHub - Relatório Gerado Automaticamente",
                    self.styles["Italic"],
                )
            )
    ...
    async def generate_comprehensive_report(
        self, start_date: str, end_date: str, format: str = "excel"
    ) -> Dict:
