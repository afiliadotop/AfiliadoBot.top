...
@router.get("")
async def get_telegram_settings(current_admin: Dict = Depends(get_current_admin)):
    """
...
async def update_telegram_settings(
    request: UpdateSettingsRequest, current_admin: Dict = Depends(get_current_admin)
):
    """
    ...
    try:
        ...
        if not request.bot_token or not request.group_chat_id:
            raise HTTPException(
                status_code=400, detail="Bot token e group chat ID são obrigatórios"
            )
        ...
        if not success:
            raise HTTPException(
                status_code=500, detail="Erro ao salvar configurações no banco de dados"
            )

        logger.info(
            f"[Telegram Settings API] Configurações atualizadas por admin:{current_admin['id']}"
        )
...
async def test_telegram_connection(
    request: TestConnectionRequest, current_admin: Dict = Depends(get_current_admin)
):
    """
    ...
    try:
        ...
        sent_message = await bot.send_message(
            chat_id=request.group_chat_id, text=test_message
        )

        logger.info(
            f"[Telegram Test] Mensagem enviada com sucesso! Message ID: {sent_message.message_id}"
        )
        ...
        if "Unauthorized" in error_msg:
            ...
            error_detail = (
                "Group Chat ID não encontrado. Verifique se o bot está no grupo."
            )
...
@router.post("/refresh")
async def refresh_telegram_cache(current_admin: Dict = Depends(get_current_admin)):
    """
    ...
    try:
        ...
        logger.info(
            f"[Telegram Settings API] Cache atualizado manualmente por admin:{current_admin['id']}"
        )
...
@router.get("/status")
async def get_telegram_status(current_admin: Dict = Depends(get_current_admin)):
    """
    ...
    try:
        ...
        logger.error(
            f"[Telegram Settings API] Erro ao buscar status: {e}", exc_info=True
        )
        raise HTTPException(status_code=500, detail=str(e))
