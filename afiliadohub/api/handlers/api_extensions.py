...
async def get_sales_funnel(
    days: int = Query(30, ge=1, le=365), store: Optional[str] = None
):
...
async def generate_recommendations(
    ...
    background_tasks.add_task(generate_recommendations_background, user_id, limit)
...
async def generate_recommendations_background(user_id: str, limit: int):
    ...
    try:
        ...
        response = (
            supabase.client.table("user_interactions")
            .select("*")
            .eq("user_id", user_id)
        ...
        for _ in range(limit):
            product = await engine.get_personalized_recommendation(
                int(user_id), chat_history
            )
            if product:
...
async def export_report(
    ...
    if report_type == "comprehensive":
        result = await exporter.generate_comprehensive_report(
...
async def detailed_health_check():
    ...
    try:
        ...
        checks["services"]["supabase"] = {"status": "disconnected", "error": str(e)}
    ...
    try:
        ...
        checks["services"]["telegram"] = {"status": "disconnected", "error": str(e)}
    ...
    checks["storage"] = {
        "status": "normal",
        "estimated_records": checks["services"]
        .get("supabase", {})
        .get("product_count", 0),
        "estimated_size_mb": checks["services"]
        .get("supabase", {})
        .get("product_count", 0)
        * 0.5,  # 0.5KB por produto
    }
