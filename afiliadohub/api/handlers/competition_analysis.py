...
class CompetitionAnalyzer:
    ...
    async def analyze_price_changes(self, product_ids: List[int]) -> Dict:
        ...
        try:
            ...
            response = (
                supabase.client.table("product_logs")
                .select("*")
    ...
    async def compare_with_competitors(self, product_url: str) -> Dict:
        ...
        try:
            ...
            for competitor in competitors:
                ...
                analysis["summary"]["best_price"] = min(
                    analysis["summary"]["best_price"], comp_price
                )
                analysis["summary"]["worst_price"] = max(
                    analysis["summary"]["worst_price"], comp_price
                )

            if competitors:
                analysis["summary"]["avg_competitor_price"] = total_price / len(
                    competitors
                )
                ...
                if current_price < analysis["summary"]["avg_competitor_price"]:
                    ...
                    analysis["recommendation_text"] = (
                        f"Considere reduzir o preço em {((current_price - analysis['summary']['avg_competitor_price']) / current_price) * 100:.1f}%"
                    )
        ...
        try:
            ...
            for keyword in keywords:
                ...
                response = (
                    ...
                    .eq("store", store)
                    .or_(
                        f"name.ilike.%{keyword}%,category.ilike.%{keyword}%,tags.cs.{{{keyword}}}"
                    )
                    .limit(20)
                    .execute()
                )
                ...
                if products:
                    ...
                    prices = [
                        p["current_price"] for p in products if p["current_price"]
                    ]
