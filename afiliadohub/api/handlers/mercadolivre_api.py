...
async def search_products(
    ...
    sort: str = Query(
        "price_asc", description="Ordenação: price_asc | price_desc | relevance"
    ),
    ...
    try:
        ...
        params = {"q": keyword, "limit": limit, "offset": 0, "condition": condition}
            ...
            products.append(
                {
                    ...
                    "shipping_free": item.get("shipping", {}).get(
                        "free_shipping", False
                    ),
        ...
        raise HTTPException(
            status_code=500, detail=f"Erro ao buscar produtos: {str(e)}"
        )
...
async def generate_link_endpoint(
    ...
    try:
        ...
        if not url.startswith(
            ("https://produto.mercadolivre.com.br", "https://www.mercadolivre.com.br")
        ):
            raise HTTPException(
                status_code=400,
                detail="URL inválida. Deve ser um produto do Mercado Livre.",
            )
        ...
        if not match:
            raise HTTPException(
                status_code=400,
                detail="Não foi possível extrair o ID do produto da URL.",
            )
