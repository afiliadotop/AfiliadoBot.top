...
@router.get(
    "", response_model=List[feedResponse] if False else List[Dict]
)  # Simplified return type for flexibility
...
async def create_feed(
    feed: FeedCreate, current_admin: Dict = Depends(get_current_admin)
):
...
async def run_feed(
    ...
    try:
        ...
        client.table("product_feeds").update(
            {"status": "running", "last_run_at": datetime.utcnow().isoformat()}
        ...
        async def run_import_task(url: str, fid: str, name: str, token: str):
            ...
            try:
                ...
                status = (
                    ...
                    else "warning" if stats else "failed"
                )
