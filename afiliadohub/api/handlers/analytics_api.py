...
async def get_performance_overview(
    days: int = Query(
        default=30, ge=1, le=365, description="Número de dias para análise"
    )
):
    """
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Erro ao buscar overview")
        )
...
async def get_top_products(
    ...
    metric: str = Query(
        default="clicks", regex="^(clicks|telegram_sends|quality_score)$"
    ),
    days: Optional[int] = Query(
        default=None, ge=1, le=365, description="Filtrar por dias"
    ),
):
    """
    ...
    result = await analytics_service.get_top_products(
        limit=limit, metric=metric, days=days
    )

    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Erro ao buscar top produtos")
        )

    return {"products": result["data"], "count": result["count"], "metric": metric}
...
async def get_store_comparison():
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Erro ao buscar comparativo")
        )

    return {"stores": result["data"], "count": result["count"]}
...
async def get_trends(
    ...
    if not result.get("success"):
        raise HTTPException(
            status_code=500, detail=result.get("error", "Erro ao buscar tendências")
        )

    return {"trends": result["data"], "count": result["count"], "period_days": days}
