...
class AdvancedAnalytics:
    ...
    async def get_sales_funnel_analysis(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            stats_response = (
                supabase.client.table("product_stats")
                .select("product_id, view_count, click_count")
                .execute()
            )

            commissions_response = (
                supabase.client.table("commissions")
            ...
            if funnel["products_added"] > 0:
                funnel["conversion_rates"]["view_to_add"] = (
                    funnel["products_viewed"] / funnel["products_added"]
                ) * 100
                funnel["conversion_rates"]["click_to_view"] = (
                    (funnel["products_clicked"] / funnel["products_viewed"]) * 100
                    if funnel["products_viewed"] > 0
                    else 0
                )
                funnel["conversion_rates"]["sale_to_click"] = (
                    (funnel["products_sold"] / funnel["products_clicked"]) * 100
                    if funnel["products_clicked"] > 0
                    else 0
                )
    ...
    def _generate_funnel_summary(self, funnel: Dict) -> Dict:
        ...
        summary = {"health": "good", "recommendations": [], "warning": []}
        ...
        if cr.get("view_to_add", 0) < 10:
            ...
            summary["recommendations"].append(
                "Aumente a visibilidade dos produtos (apenas {:.1f}% dos produtos estão sendo vistos)".format(
                    cr["view_to_add"]
                )
            )
        ...
        if cr.get("click_to_view", 0) < 5:
            summary["health"] = "poor"
            summary["recommendations"].append(
                "Melhore a atratividade dos produtos (taxa de cliques baixa: {:.1f}%)".format(
                    cr["click_to_view"]
                )
            )
        ...
        if cr.get("sale_to_click", 0) < 1:
            summary["warning"].append(
                "Taxa de conversão muito baixa ({:.2f}%). Considere ajustar preços ou melhorar descrições.".format(
                    cr["sale_to_click"]
                )
            )
        ...
        if summary["health"] == "good":
            summary["recommendations"].append(
                "Funil saudável! Continue com a estratégia atual."
            )
    ...
    async def generate_performance_report(self, start_date: str, end_date: str) -> Dict:
        ...
        try:
            ...
            stats = supabase.client.table("product_stats").select("*").execute()
            ...
            if not df_products.empty:
                ...
                if not df_commissions.empty:
                    commissions_by_product = (
                        df_commissions.groupby("product_id")
                        .agg(
                            {
                                "sale_amount": "sum",
                ...
                report = {
                    ...
                    "charts": {
                        "daily_trends": await self._generate_daily_trends(
                            start_date, end_date
                        ),
                        "store_performance": await self._generate_store_chart(
                            df_merged
                        ),
                    },
                }
    ...
    def _calculate_summary_metrics(self, df: pd.DataFrame) -> Dict:
        ...
        metrics = {
            ...
            "active_products": (
                df["is_active"].sum() if "is_active" in df.columns else 0
            ),
            "avg_price": (
                df["current_price"].mean() if "current_price" in df.columns else 0
            ),
            "avg_discount": (
            ...
            "total_clicks": (
                df["click_count"].sum() if "click_count" in df.columns else 0
            ),
            "total_sales": (
                df["sales_count"].sum() if "sales_count" in df.columns else 0
            ),
            "total_revenue": (
                df["sale_amount"].sum() if "sale_amount" in df.columns else 0
            ),
            "total_commission": (
            ...
            "total_telegram_sends": (
                df["telegram_send_count"].sum()
                if "telegram_send_count" in df.columns
                else 0
            ),
        }
        ...
        if metrics["total_views"] > 0:
            metrics["click_through_rate"] = (
                metrics["total_clicks"] / metrics["total_views"]
            ) * 100
        else:
        ...
        if metrics["total_clicks"] > 0:
            metrics["conversion_rate"] = (
                metrics["total_sales"] / metrics["total_clicks"]
            ) * 100
        else:
    ...
    def _analyze_by_store(self, df: pd.DataFrame) -> Dict:
        ...
        for store in df["store"].unique():
            ...
            store_analysis[store] = {
                ...
                "total_sales": (
                    store_df["sales_count"].sum()
                    if "sales_count" in store_df.columns
                    else 0
                ),
                "total_revenue": (
                    store_df["sale_amount"].sum()
                    if "sale_amount" in store_df.columns
                    else 0
                ),
                "click_through_rate": (
                    (store_df["click_count"].sum() / store_df["view_count"].sum() * 100)
                    if store_df["view_count"].sum() > 0
                    else 0
                ),
            }
    ...
    async def _generate_daily_trends(self, start_date: str, end_date: str) -> Dict:
        ...
        try:
            ...
            response = supabase.client.rpc(
                "get_daily_trends", {"p_start_date": start_date, "p_end_date": end_date}
            ).execute()
