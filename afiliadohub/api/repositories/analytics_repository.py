...
class AnalyticsRepository:
    ...
    async def get_total_clicks(
        ...
        try:
            query = self.client.table("product_stats").select(
                "click_count", count="exact"
            )
            ...
            total = (
                sum(item.get("click_count", 0) for item in response.data)
                if response.data
                else 0
            )
            return total
    ...
    async def get_top_products(
        ...
        try:
            ...
            if order_by == "quality_score":
                ...
                products.sort(
                    key=lambda x: x.get("telegram_send_count", 0), reverse=True
                )
            else:  # clicks
        ...
        try:
            ...
            for p in response.data:
                ...
                if stats and len(stats) > 0:
                    ...
                    stores[store]["total_telegram_sends"] += stats[0].get(
            ...
            for store_data in stores.values():
                ...
                if product_count > 0:
                    store_data["avg_clicks_per_product"] = round(
                        store_data["total_clicks"] / product_count, 2
                    )
                    ...
                    if store_data["total_telegram_sends"] > 0:
                        store_data["ctr"] = round(
                            (
                                store_data["total_clicks"]
                                / store_data["total_telegram_sends"]
                            )
                            * 100,
                            2,
                        )
        ...
        try:
            ...
            for stat in response.data:
                ...
                if date_str not in daily_stats:
                    daily_stats[date_str] = {"date": date_str, "clicks": 0}
    ...
    async def get_overview_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if quality_response.data:
                avg_quality = sum(
                    p.get("quality_score", 0) for p in quality_response.data
                ) / len(quality_response.data)
            else:
                avg_quality = 0
            ...
            if stats_response.data:
                total_sends = sum(
                    s.get("telegram_send_count", 0) for s in stats_response.data
                )
                total_all_clicks = sum(
                    s.get("click_count", 0) for s in stats_response.data
                )
                avg_ctr = (
                    (total_all_clicks / total_sends * 100) if total_sends > 0 else 0
                )
            else:
                avg_ctr = 0
