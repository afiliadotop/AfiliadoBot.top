...
class AnalyticsService:
    ...
    async def get_performance_overview(self, days: int = 30) -> Dict[str, Any]:
        """
    ...
    async def get_top_products(
        self, limit: int = 10, metric: str = "clicks", days: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            products = await self.repository.get_top_products(
                limit=limit, order_by=metric, date_from=date_from
            )
                ...
                formatted.append(
                    {
                        ...
                        "ctr": (
                            round(
                                (
                                    p.get("click_count", 0)
                                    / p.get("telegram_send_count", 1)
                                )
                                * 100,
                                2,
                            )
                            if p.get("telegram_send_count", 0) > 0
                            else 0
                        ),
                    }
                )

            return {"success": True, "data": formatted, "count": len(formatted)}
    ...
    async def get_trends(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            while current_date <= end_date:
                ...
                all_days.append(
                    {"date": date_str, "clicks": data_dict.get(date_str, 0)}
                )
                current_date += timedelta(days=1)

            return {"success": True, "data": all_days, "count": len(all_days)}
    ...
    def should_import_product(
        self, product: Dict[str, Any], min_quality: int = 60
    ) -> tuple[bool, int]:
        """
