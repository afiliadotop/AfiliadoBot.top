...
class ProductService(BaseService[Product]):
    ...
    def __init__(
        ...
        self.affiliate_service: AffiliateService = (
            affiliate_service or AffiliateService()
        )
    ...
    def get_products(
        self, filters: Optional[ProductFilter] = None, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
    ...
    def _apply_filters(self, filters: ProductFilter, limit: int) -> List:
        ...
        if filters.min_price or filters.max_price:
            return self.repository.get_by_price_range(
                filters.min_price, filters.max_price, limit
            )

        # Commission filter
        if filters.min_commission:
            return self.repository.get_by_commission_range(
                filters.min_commission, limit
            )
    ...
    def _enrich_product(self, product: dict) -> dict:
        ...
        if current_price and original_price:
            ...
            if self.price_history_repo and product_id:
                try:
                    historical_avg = self.price_history_repo.get_historical_average(
                        product_id
                    )
        ...
        if product.get("commission_rate") and current_price:
            commission = (
                float(current_price) * float(product["commission_rate"])
            ) / 100
