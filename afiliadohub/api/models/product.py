...
class ProductBase(BaseModel):
    ...
    discount_percentage: Optional[int] = Field(
        None, ge=0, le=100, description="Porcentagem de desconto"
    )
    ...
    coupon_code: Optional[str] = Field(
        None, max_length=100, description="Código do cupom"
    )
    coupon_expiry: Optional[datetime] = Field(
        None, description="Data de expiração do cupom"
    )

    tags: Optional[List[str]] = Field(
        default_factory=list, description="Tags do produto"
    )
    ...
    def validate_discount(cls, v, values):
        ...
        if v is not None and "original_price" in values and values["original_price"]:
            calculated_discount = int(
                (
                    (values["original_price"] - values["current_price"])
            ...
            if abs(v - calculated_discount) > 5:  # Tolerância de 5%
                raise ValueError(
                    f"Desconto inconsistente. Esperado: {calculated_discount}%, Recebido: {v}%"
                )
        return v
