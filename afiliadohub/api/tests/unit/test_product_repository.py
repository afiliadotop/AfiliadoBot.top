...
class TestProductRepository:
    ...
    def test_get_all(
        self, product_repository, mock_supabase_client, sample_products_list
    ):
        """Test get all products"""
        # Arrange
        mock_supabase_client.table().select().limit().execute.return_value.data = (
    ...
    def test_get_all_with_filters(
        self, product_repository, mock_supabase_client, sample_product
    ):
        """Test get all with filters"""
        # Arrange
        mock_supabase_client.table().select().eq().limit().execute.return_value.data = [
            sample_product
        ]
    ...
    def test_search_by_name(
        ...
        # Arrange
        mock_supabase_client.table().select().ilike().limit().execute.return_value.data = [
            sample_product
        ]
    ...
    def test_get_by_price_range(
        self, product_repository, mock_supabase_client, sample_products_list
    ):
        """Test get products by price range"""
        # Arrange
        mock_supabase_client.table().select().gte().lte().limit().execute.return_value.data = (
            sample_products_list
        )

        # Act
        result = product_repository.get_by_price_range(
            min_price=50.0, max_price=150.0, limit=100
        )

        # Assert
        assert len(result) == 2

    def test_get_by_commission_range(
        self, product_repository, mock_supabase_client, sample_product
    ):
        """Test get products by minimum commission"""
        # Arrange
        mock_supabase_client.table().select().gte().limit().execute.return_value.data = [
            sample_product
        ]

        # Act
        result = product_repository.get_by_commission_range(
            min_commission=8.0, limit=100
        )
