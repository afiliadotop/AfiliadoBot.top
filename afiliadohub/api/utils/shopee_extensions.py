    ...
    async def acquire(self) -> None:
        ...
        async with self._lock:
            ...
            if len(self._requests) >= self.MAX_REQUESTS_PER_HOUR:
                ...
                if wait_time > 0:
                    logger.warning(
                        f"⚠️ Rate limit atingido! " f"Aguardando {wait_time:.0f}s..."
                    )
...
class ScrollIdPaginator:
    ...
    async def fetch_all_pages(
        self, max_pages: Optional[int] = None, delay_between_pages: float = 1.0
    ) -> List[Dict[str, Any]]:
        """
...
async def get_all_conversions(
    client, start_timestamp: int, end_timestamp: int, max_pages: Optional[int] = None
) -> List[Dict[str, Any]]:
    """
