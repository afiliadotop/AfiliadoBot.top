...
class ShopeeProductImporter:
    ...
    def _map_shopee_to_product(self, shopee_product: Dict[str, Any]) -> Dict[str, Any]:
        ...
        product_data = {
            ...
            "rating": (
                float(shopee_product.get("rating", 0))
                if shopee_product.get("rating")
                else None
            ),
            ...
            "shop_rating": (
                float(shopee_product.get("shopRating", 0))
                if shopee_product.get("shopRating")
                else None
            ),
            "is_active": True,
            "is_featured": discount_percentage >= 50
            or shopee_product.get("commissionRate", 0) >= 10,
        }

        return product_data

    async def import_all_products(
        self, limit: int = 100, min_commission: float = 0.0
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"[ShopeeImporter] Iniciando importação (limit={limit}, min_commission={min_commission}%)"
        )
        ...
        try:
            ...
            shopee_products = await self.shopee.get_products(
                min_commission=min_commission, limit=limit
            )

            logger.info(
                f"[ShopeeImporter] {len(shopee_products)} produtos recebidos da API"
            )
            ...
            for shopee_product in shopee_products:
                try:
                    ...
                    if existing:
                        ...
                        stats["updated"] += 1
                        logger.debug(
                            f"[ShopeeImporter] Produto {existing['id']} atualizado"
                        )
                    else:
                        # Insere novo produto
                        ...
                        stats["imported"] += 1
                        logger.debug(
                            f"[ShopeeImporter] Produto {result['id']} importado"
                        )
    ...
    async def update_existing_products(self, hours_old: int = 24) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"[ShopeeImporter] Atualizando produtos desatualizados (>{hours_old}h)"
        )
        ...
        try:
            ...
            response = self.supabase.client.rpc(
                "get_outdated_shopee_products", {"hours_old": hours_old}
            ).execute()

            outdated_products = response.data
            logger.info(
                f"[ShopeeImporter] {len(outdated_products)} produtos para atualizar"
            )
            ...
            shopee_map = {p.get("productId"): p for p in shopee_products}
                ...
                if shopee_id in shopee_map:
                    try:
                        ...
                        stats["error_messages"].append(
                            f"Erro ao atualizar {shopee_id}: {e}"
                        )
    ...
    async def sync_brand_offers(self) -> Dict[str, Any]:
        ...
        stats = {"imported": 0, "errors": 0, "error_messages": []}
    ...
    async def _update_product(self, product_id: str, new_data: Dict[str, Any]):
        ...
        self.supabase.client.table("products").update(new_data).eq(
            "id", product_id
        ).execute()
...
async def run_shopee_import(
    import_type: str = "all", limit: int = 100, min_commission: float = 0.0
) -> Dict[str, Any]:
    """
    ...
    if import_type == "all":
        return await importer.import_all_products(
            limit=limit, min_commission=min_commission
        )
