...
class FeedManager:
            ...
            for feed in feeds:
                ...
                if not feed.get("last_run_at"):
                    ...
                    last_run = datetime.fromisoformat(
                        feed["last_run_at"].replace("Z", "+00:00")
                    )
    ...
    async def _process_single_feed(self, feed: Dict):
        ...
        store_id = feed[
            "store_id"
        ]  # Precisamos mapear id para nome da store se o CSVImporter pedir string

        logger.info(f"[FEED] Iniciando processamento do feed: {name}")

        try:
            # Atualiza status para running
            self.supabase.client.table("product_feeds").update(
                {"status": "running", "last_run_at": datetime.now().isoformat()}
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    url, timeout=1800
                ) as resp:  # 30 min timeout para arquivos grandes
                    if resp.status != 200:
            ...
            if store_id:
                store_res = (
                    self.supabase.client.table("stores")
                    .select("name")
                    .eq("id", store_id)
                    .single()
                    .execute()
            ...
            stats = await importer.process_csv_upload(
                file_content=file_content, store=store_name, send_to_telegram=False
            )

            # Atualiza status para success
            self.supabase.client.table("product_feeds").update(
                {"status": "success", "updated_at": datetime.now().isoformat()}
            ).eq("id", feed_id).execute()
