...
class TelegramSettingsManager:
    ...
    def get_settings(self, force_refresh: bool = False) -> Dict:
        ...
        should_refresh = (
            ...
            or not self._last_refresh
            or (now - self._last_refresh) > timedelta(minutes=self._ttl_minutes)
        )
    ...
    def _refresh_from_db(self):
        ...
        try:
            ...
            if result.data and len(result.data) > 0:
                ...
                logger.warning(
                    "[Telegram Settings] Nenhuma configuração ativa encontrada"
                )
                self._settings = {
                    "bot_token": "",
                    "group_chat_id": "",
            ...
            logger.error(
                f"[Telegram Settings] Erro ao carregar do banco: {e}", exc_info=True
            )
    ...
    def update_settings(
        ...
        try:
            ...
            supabase.client.table("telegram_settings").update({"is_active": False}).eq(
                "is_active", True
            ).execute()
            ...
            result = supabase.client.table("telegram_settings").insert(data).execute()

            # Refresh cache imediatamente
            self._refresh_from_db()

            logger.info(
                f"[Telegram Settings] Configurações atualizadas por user {user_id}"
            )
            return True
    ...
    def is_configured(self) -> bool:
        ...
        # Check DB first
        if (
            settings.get("is_active")
            and settings.get("bot_token")
            and settings.get("group_chat_id")
        ):
            return True
        ...
        return bool(
            os.getenv("TELEGRAM_BOT_TOKEN") and os.getenv("TELEGRAM_CHANNEL_ID")
        )
