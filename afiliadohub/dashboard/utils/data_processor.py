...
class DataProcessor:
    ...
    def process_csv_data(df: pd.DataFrame, store: str) -> List[Dict[str, Any]]:
        ...
        for _, row in df.iterrows():
            try:
                ...
                name = DataProcessor._extract_field(
                    row, ["name", "product", "title", "nome"]
                )
                link = DataProcessor._extract_field(
                    row, ["link", "url", "affiliate_link"]
                )
                price = DataProcessor._extract_price(
                    row, ["price", "current_price", "preco"]
                )
                ...
                product = {
                    ...
                    "original_price": DataProcessor._extract_price(
                        row, ["original_price", "old_price", "preco_original"]
                    ),
                    "category": DataProcessor._extract_field(
                        row, ["category", "categoria"]
                    ),
                    "image_url": DataProcessor._extract_field(
                        row, ["image", "image_url", "imagem"]
                    ),
                    "source": "csv_import",
                    "is_active": True,
    ...
    def aggregate_daily_stats(data: List[Dict]) -> Dict:
        ...
        stats = {
            ...
            "total_value": (
                df["current_price"].sum() if "current_price" in df.columns else 0
            ),
            "with_discount": (
                df["discount_percentage"].notnull().sum()
