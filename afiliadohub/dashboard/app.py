...
async def load_dashboard_data():
    ...
    daily_trends = await analytics._generate_daily_trends(
        (datetime.now() - timedelta(days=7)).isoformat(), datetime.now().isoformat()
    )
...
def main():
    ...
    with col_left:
        st.plotly_chart(
            create_sales_funnel_chart(funnel_metrics), use_container_width=True
        )
    ...
    with col_right:
        ...
        st.plotly_chart(
            create_store_performance_chart(store_data), use_container_width=True
        )
    ...
    if products:
        ...
        display_df = df_products[
            ["name", "store", "current_price", "discount_percentage", "created_at"]
        ].copy()
