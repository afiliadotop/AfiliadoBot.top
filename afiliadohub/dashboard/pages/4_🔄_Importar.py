    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            with col1:
                ...
                price_col = st.selectbox("Coluna do Preço", df.columns.tolist(),
...
with tab2:
    ...
    import_method = st.radio(
        ...
        ["CSV Diário", "API Shopee", "Web Scraping"]
    )
        ...
        if st.button("🕷️ Iniciar Scraping", type="primary"):
            with st.spinner(f"Buscando '{search_query}' na Shopee..."):
                try:
                    ...
                    st.success(f"✅ Encontrados {len(sample_data) * max_pages} produtos!")
...
with tab3:
    ...
    with st.expander("🔔 Notificações"):
        ...
        if email_notifications:
            email_address = st.text_input("Email para notificações:", value="seu@email.com")
