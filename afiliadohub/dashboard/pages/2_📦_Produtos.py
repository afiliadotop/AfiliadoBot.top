...
st.set_page_config(
    page_title="Gerenciar Produtos - AfiliadoHub", page_icon="📦", layout="wide"
)
    ...
    if st.button("🔍 Buscar Produtos", type="primary"):
        with st.spinner("Buscando produtos..."):
            try:
                ...
                query = query.gte("current_price", price_range[0]).lte(
                ...
                if response.data:
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            ...
                            "discount_percentage": st.column_config.TextColumn(
                                "Desconto", width="small"
                            ),
                            "category": st.column_config.TextColumn(
                                "Categoria", width="medium"
                            ),
                            "created_at": st.column_config.TextColumn(
                                "Adicionado", width="medium"
                            ),
                        },
                    )
    ...
    with st.form("add_product_form"):
        ...
        with col2:
            current_price = st.number_input(
                "Preço Atual (R$)*", min_value=0.01, value=99.90, step=0.01
            )
            original_price = st.number_input(
                "Preço Original (R$)", min_value=0.00, value=0.00, step=0.01
            )
            ...
            if not name or not affiliate_link or not current_price:
                ...
                try:
                    product_data = {
                        ...
                        "original_link": (
                            original_link if original_link else affiliate_link
                        ),
                        "current_price": float(current_price),
                        "original_price": (
                            float(original_price) if original_price > 0 else None
                        ),
                        ...
                        "coupon_expiry": (
                            coupon_expiry.isoformat() if coupon_expiry else None
                        ),
                        "tags": (
                    ...
                    if response.data:
                        st.success(
                            f"✅ Produto '{name}' adicionado com sucesso! ID: {response.data[0]['id']}"
                        )
                    ...
                    for update in updates:
                        try:
                            ...
                            if pd.notnull(product_id):
                                supabase.table("products").update(update).eq(
                                    "id", int(product_id)
                                ).execute()
                                success_count += 1
                        except Exception as e:
...
with tab4:
    ...
    if removal_option == "Produtos Inativos":
        try:
            ...
            if inactive_count > 0 and st.button(
                "🗑️ Remover Produtos Inativos", type="secondary"
            ):
        ...
        try:
            ...
            if old_count > 0 and st.button(
                f"🗑️ Remover Produtos Antigos", type="secondary"
            ):
                with st.spinner("Removendo produtos antigos..."):
                    supabase.table("products").delete().lt(
        ...
        try:
            ...
            if store_count > 0 and st.button(
                f"🗑️ Remover Todos de {store_to_remove}", type="secondary"
            ):
                with st.spinner(f"Removendo produtos da {store_to_remove}..."):
                    supabase.table("products").delete().eq(
                        "store", store_to_remove
                    ).execute()
        ...
        password = st.text_input(
            "Digite 'CONFIRMAR-DELETE' para prosseguir:", type="password"
        )
